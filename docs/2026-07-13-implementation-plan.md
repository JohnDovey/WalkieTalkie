...
- `android/` scaffolded per `android-build.mdc`/ClonesApp precedent; PTT UI (Compose), foreground service (`microphone` type) hosting core's mesh/registry/mDNS lifecycle, BLE presence bridge (`BluetoothLeScanner`/`AdvertiseCallback`), GPS via `FusedLocationProviderClient` — `assembleDebug` succeeds (verified 2026-07-13, ~15min first build), producing a real `app-debug.apk` including the gomobile-built `libgojni.so`.
- `MediaCodec` Opus capture/playback implemented (`audio/OpusMicSource.kt`, `audio/OpusSpeakerSink.kt`) — mic via `AudioRecord` → MediaCodec `audio/opus` encoder; incoming frames → per-peer MediaCodec `audio/opus` decoder → `AudioTrack`. **minSdk bumped from 26 to 29** (differs from the ClonesApp precedent) because MediaCodec's Opus *encoder* (as opposed to the older decoder) was only added in Android 10. **Not yet verified against real hardware** — this dev environment has no attached Android device/emulator with a working mic/speaker, so only compilation is confirmed; the encoder/decoder dequeue-loop timing (MediaCodec doesn't guarantee 1 output per 1 input call) should be watched closely on first real-device test.
- **Verify** (not yet done, blocked on real hardware): Android + one desktop instance on the same Wi-Fi discover each other and exchange PTT audio both ways; with Android Wi-Fi off, confirm a third device's BLE scan still reports the phone's presence to the server (validates offline-forwarding end to end).
